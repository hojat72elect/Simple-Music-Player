...
import ca.hojat.smart.musicplayer.shared.extensions.countdown
import ca.hojat.smart.musicplayer.shared.extensions.getProperTextColor
import ca.hojat.smart.musicplayer.shared.helpers.DEFAULT_PASSWORD_COUNTDOWN
import ca.hojat.smart.musicplayer.shared.helpers.MAX_PASSWORD_RETRY_COUNT
...
abstract class BaseSecurityTab(context: Context, attrs: AttributeSet) :
    ...
    private fun updateCountdownText(count: Int) {
        ...
        if (count > 0) {
            updateTitle(
                context.getString(R.string.too_many_incorrect_attempts, count),
                context.getColor(R.color.md_red)
            )
        } else {
            updateTitle(context.getString(defaultTextRes), context.getProperTextColor())
        }
